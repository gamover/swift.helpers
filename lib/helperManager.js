...
HelperManager.prototype.addHelper = function addHelper (helperName, helper)
{
    ...
    if (typeof helperName !== 'string')
        throw new $swiftErrors.TypeError('Недопустимый тип имени помощника (ожидается: "string", принято: "' + typeof helperName + '").');
    if (!helperName.length)
        throw new $swiftErrors.ValueError('Недопустимое значение имени помощника.');
    if (helperName in this._helpers)
        throw new $swiftErrors.SystemError('Помощник с именем "' + helperName + '" уже существует.');
    if (!$swiftUtils.type.isObject(helper))
        throw new $swiftErrors.TypeError('Недопустимый тип помощника (ожидается: "object", принято: "' + typeof helperName + '").');
    //
    // добавление помощника
    //
