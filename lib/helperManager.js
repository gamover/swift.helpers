...
HelperManager.prototype.addHelper = function addHelper (helperName, helper)
{
    ...
    if (typeof helperName !== 'string')
        throw new $swiftErrors.TypeError('HelperManager: недопустимый тип имени помощника (ожидается: "string", принято: "' + typeof helperName + '")');
    if (!helperName.length)
        throw new $swiftErrors.ValueError('HelperManager: пустое значение имени помощника');
    if (helperName in this._helpers)
        throw new $swiftErrors.SystemError('HelperManager: помощник с именем "' + helperName + '" уже существует');
    if (!$swiftUtils.type.isObject(helper))
        throw new $swiftErrors.TypeError('HelperManager: Недопустимый тип помощника (ожидается: "object", принято: "' + typeof helperName + '")');
    //
    // добавление помощника
    //
...
HelperManager.prototype.removeHelper = function removeHelper (helperName)
{
    ...
    if (typeof helperName !== 'string')
        throw new $swiftErrors.TypeError('HelperManager: недопустимый тип имени помощника (ожидается: "string", принято: "' + typeof helperName + '")');
    if (!helperName.length)
        throw new $swiftErrors.ValueError('HelperManager: недопустимое значение имени помощника');
    //
    // удаление помощника
    //
