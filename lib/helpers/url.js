...
UrlHelper.prototype.resolve = function resolve (alias, params)
{
    ...
    if (!(alias in this._routes))
        throw new $swiftErrors.SystemError('Не найден маршрут с псевдонимом "' + alias + '".');
    ...
    if (isRegexp && !path.match(new RegExp(route.path)))
        throw new $swiftErrors.ValueError('Ссылка "' + path + '" не соответсвует регулярному выражению "' + route.path + '" в маршруте с псевдонимом "' + alias + '".');
